...
class TrajectoryRelationFusion(nn.Module):
    ...
    def _sparse_writeback(self, patch_feat: torch.Tensor, traj_upd: torch.Tensor, points: torch.Tensor, vis: torch.Tensor) -> torch.Tensor:
        ...
        for t in range(tdim):
            ...
            delta = delta / norm
            updated[:, t] = updated[:, t] + self.writeback_scale * delta
        return updated
    ...
    def forward(
        ...
        traj_upd = self.proj_out(h_rel)
        patch_upd = self._sparse_writeback(patch_feat, traj_upd, points, vis.bool())
        return patch_upd, h_rel
