...
# pylint: disable=no-member,possibly-used-before-assignment,redefined-outer-name
"""Transform"""
import math
import random
...
def color_normalization(images, mean, stddev):
    ...
    out_images = torch.zeros_like(images)
    for idx, (mean_val, std_val) in enumerate(zip(mean, stddev)):
        # C H W
        if len(images.shape) == 3:
            out_images[idx] = (images[idx] - mean_val) / std_val
        elif len(images.shape) == 4:
            out_images[:, idx] = (images[:, idx] - mean_val) / std_val
        else:
            raise NotImplementedError(
