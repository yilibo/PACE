...
line_thickness=1, max_cluster_id=None, trail_history_frames=None):
    """
    This function calculates the median motion of the background, which is subsequently
    subtracted from the foreground motion. This subtraction process "stabilizes" the camera and
    improves the interpretability of the foreground motion trails.
    ...
    frames = []
    if trail_history_frames is None or trail_history_frames <= 0:
        back_history = kpts.shape[0]
    else:
        back_history = int(trail_history_frames)
...
def _resolve_frame_index(num_imgs, anchor_frame):
    """Resolve anchor frame selector to a concrete index."""
    if isinstance(anchor_frame, int):
        return max(0, min(num_imgs - 1, anchor_frame))
    if isinstance(anchor_frame, str):
        anchor = anchor_frame.strip().lower()
        if anchor == "first":
            return 0
        if anchor == "middle":
            return num_imgs // 2
        ...
        try:
            idx = int(anchor)
            return max(0, min(num_imgs - 1, idx))
        except ValueError:
            return 0
    return 0


def vis_trail_middle_frame(video, kpts, kpts_vis, kpts_queries=None, point_to_take=None,
                           cluster_ids=None, line_thickness=1, max_cluster_id=None,
                           draw_endpoints=True, require_visibility=True,
                           anchor_frame="first"):
    """Render all point trajectories on one anchor frame and return a single image."""
    color_map = cm.get_cmap("jet")

    if cluster_ids is not None:
        if max_cluster_id is None:
            max_cluster_id_to_use = max(cluster_ids) if len(cluster_ids) > 0 else 1
        else:
            max_cluster_id_to_use = max_cluster_id
    else:
        max_cluster_id_to_use = 1
    ...
    num_imgs, num_pts = kpts.shape[:2]
    base_idx = _resolve_frame_index(num_imgs, anchor_frame)
    img_curr = video[base_idx].copy()

    for j in range(num_pts):
        start_t = max(int(kpts_queries[j]), 0)
        if start_t >= num_imgs - 1:
            continue

        if (kpts_vis[start_t:, j] == 0).all():
            continue

        if cluster_ids is not None:
            color = np.array(color_map(cluster_ids[j] / max(1, max_cluster_id_to_use))[:3]) * 255
        else:
            color = np.array(color_map(j / max(1, float(num_pts - 1)))[:3]) * 255
        ...
        for t in range(start_t, num_imgs - 1):
            if require_visibility and ((kpts_vis[t, j] == 0) or (kpts_vis[t + 1, j] == 0)):
                continue
            pt1 = kpts[t, j]
            pt2 = kpts[t + 1, j]
            p1 = (int(round(pt1[0])), int(round(pt1[1])))
            p2 = (int(round(pt2[0])), int(round(pt2[1])))
            cv2.line(img_curr, p1, p2, color_int, thickness=line_thickness, lineType=16)

        if draw_endpoints:
            if require_visibility:
                visible_indices = np.where(kpts_vis[:, j] > 0)[0]
                if len(visible_indices) > 0:
                    endpoint_indices = [visible_indices[0], visible_indices[-1]]
                else:
                    endpoint_indices = []
            else:
                endpoint_indices = [start_t, num_imgs - 1]
            for t in endpoint_indices:
                pt = kpts[t, j]
                p = (int(round(pt[0])), int(round(pt[1])))
                cv2.circle(img_curr, p, 2, color_int, -1, lineType=16)

    return Image.fromarray(img_curr.astype(np.uint8))
