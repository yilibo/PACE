...
class CLIP(nn.Module):
        ...
        if isinstance(vision_layers, (tuple, list)):
            ...
            if vision_model != "VisionTransformer":
                raise NotImplementedError(
                    f"Unsupported vision model '{vision_model}'. "
                    "This repository keeps only the base VisionTransformer path."
                )
    ...
    def encode_image(self, image, trajectory_info=None):
        del trajectory_info
    ...
    def forward(self, image, text, trajectory_info=None):
        image_features = self.encode_image(image, trajectory_info=trajectory_info)
...
def build_model(state_dict: dict, design_details=None):
    if design_details is None:
        context_length = int(state_dict["positional_embedding"].shape[0]) if "positional_embedding" in state_dict else 77
        design_details = {
